...
// Genera la consulta
/*$select = "select p.id_producto, p.codigo, p.nombre, p.nombre_factura, p.cantidad_minima, u.unidad, u.sigla, c.categoria";
...
$join = "";*/
$select = "select p.id_producto, p.codigo, p.nombre, p.nombre_factura, p.cantidad_minima, c.categoria";
...
foreach ($almacenes as $nro => $almacen) {
	...
	$join = $join . " 	LEFT JOIN (
							SELECT d.producto_id, sum(d.cantidad*u.tamanio) as ingresos$id 
							...
							WHERE i.almacen_id = $id and i.transitorio = 0
							GROUP BY d.producto_id
						) as e$id on e$id.producto_id = p.id_producto";
	
	$join = $join . " 	LEFT JOIN (select d.producto_id, sum(d.cantidad*u.tamanio) as egresos$id 
							FROM inv_egresos_detalles d 
							LEFT JOIN inv_egresos e on (e.id_egreso = d.egreso_id AND estado='V') 
							LEFT JOIN inv_asignaciones a ON a.id_asignacion = d.asignacion_id
							LEFT JOIN inv_unidades u ON u.id_unidad = a.unidad_id 
							WHERE e.almacen_id = $id 
							GROUP BY d.producto_id
						) as s$id on s$id.producto_id = p.id_producto ";
    //ORDER BY c.id_categoria ASC, p.codigo";
}
	...
	<table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
		...
		<tfoot>
			<tr class="active">
				...
				<?php } ?>
				<th class="text-nowrap align-middle" data-datafilter-filter="true">Total existencias</th>
		...
		<tbody>
			...
			<tr>
				...
				<!--td class="text-nowrap"><?php // escape($producto['unidad']); ?></td-->
			</tr>
			<?php } ?>
		</tbody>
	</table>
