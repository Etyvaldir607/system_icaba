...
foreach ($almacenes as $nro => $almacen) {
	...
	$join = $join . " left join (select d.producto_id, sum(d.cantidad*u.tamanio) as ingresos$id from inv_ingresos_detalles d left join inv_ingresos i on i.id_ingreso = d.ingreso_id LEFT JOIN inv_asignaciones asig ON
            ...
            LEFT JOIN inv_unidades u ON
                u.id_unidad = asig.unidad_id where i.almacen_id = $id group by d.producto_id) as e$id on e$id.producto_id = p.id_producto";
	$join = $join . " left join (select d.producto_id, sum(d.cantidad*tamanio) as egresos$id from inv_egresos_detalles d left join inv_egresos e on (e.id_egreso = d.egreso_id AND estado='V') LEFT JOIN inv_asignaciones asig ON
                d.asignacion_id=asig.id_asignacion
            LEFT JOIN inv_unidades u ON
	...
	<table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-nowrap text-right" data-cantidad="<?= $producto['id_producto'].'-'.$almacen['id_almacen']; ?>">
					<a href="#" data-stock="<?= $stock.'|'.base64_encode($producto['nombre']).'|'.$almacen['almacen'].'|'.$producto['id_producto'].'|'.$almacen['id_almacen']; ?>">
						<strong class="<?= ($stock < escape($producto['cantidad_minima'])) ? 'text-danger' : 'text-success'; ?>"><?= $stock; ?></strong></td>
					</a> 
...
$(function () {	
	...
	$("#table tbody").on('click','[data-stock]',function(e){
		e.preventDefault();
		let datos = $(this).data('stock').split('|');
		$("[data-producto]").text(atob(datos[1]));
		$("[data-almacen]").text(datos[2]);
		$("#stock_nuevo").val(datos[0]);
		$("#stock_actual").text(datos[0]);
		$("#stock").val(datos[0]).focus();

		$("#producto").val(datos[3]);
		$("#almacen").val(datos[4]);
		$("#modal_ajuste").modal('show');
	});

	$("#form_ajuste").on('submit',function(e){
		e.preventDefault();
	});
