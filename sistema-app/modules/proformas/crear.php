...
$clientes = $db->query("SELECT * from 
						(
							(select nombre_cliente, nit_ci, telefono, direccion from inv_egresos) 
							union 
							(select nombre_cliente, nit_ci, telefono, direccion from inv_proformas)
						) c 
						group by c.nombre_cliente, c.nit_ci, c.telefono, c.direccion 
						order by c.nombre_cliente asc, c.nit_ci asc"
					)->fetch();

$clientes = $db->query("SELECT id_egreso, c.nombre_cliente, c.nit_ci, c.telefono, c.escalafon
						FROM inv_clientes c
...
if ($permiso_sucursal) {
	...
	if($id_sucursal==0){
		...
		header("Location: ?/proformas/seleccionar_sucursal");
				...
				<form id="formulario" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							...
							<select name="cliente" id="cliente" class="form-control text-uppercase" data-validation="letternumber" data-validation-allowing="-+./&() " data-validation-optional="true">
								...
								<option value="<?= escape($cliente['nit_ci']) . '|' . escape($cliente['nombre_cliente']) . '|' . escape($cliente['telefono']) . '|' . escape($cliente['escalafon']); ?>"><?= escape($cliente['nit_ci']) . ' &mdash; ' . escape($cliente['nombre_cliente']); ?></option>
						...
						<table id="ventas" class="table table-bordered table-condensed table-striped table-hover margin-none">
							...
							<thead>
								...
								<tr class="active">
									...
									<th class="text-nowrap text-center">UNIDAD DE<br>MEDIDA</th>
					...
					<div class="form-group">
						...
						<div class="col-xs-12">
							...
							<input type="text" name="almacen_id" value="<?= $almacenes['id_almacen']; ?>" class="translate" tabindex="-1" data-validation="required number" data-validation-error-msg="El almacén no esta definido">
	...
	<div class="col-md-6">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<form method="post" action="?/proformas/buscar/<?php echo $id_almacen; ?>" id="form_buscar_0" class="margin-bottom" autocomplete="off">
				...
				<form method="post" action="?/proformas/buscar/<?php echo $id_almacen; ?>" id="form_buscar_1" class="margin-bottom" autocomplete="off">
...
<h2 class="btn-success position-left-bottom display-table btn-circle margin-all display-table hidden-xs" data-toggle="tooltip" data-title="Esto es una proforma" data-placement="right">
...
<table class="hidden">
	<tbody id="fila_filtrar" data-negativo="<?= imgs; ?>/" data-positivo="<?= files; ?>/productos/">
		<tr data-busqueda="">
			<td class="align-middle text-center width-none">
			...
			<td class="align-middle" data-codigo=""></td>
			<td class="align-middle cursor-pointer" onclick="permutar(this)" style="width: 5%;">
				<span class="hidden" data-nombre=""></span>
				<em></em>
				<br>
				<small class="text-primary occult"></small>
			</td>
			<td class="align-middle text-right" data-stock=""></td>
				...
				if (productos.length) {
					...
					for (var i in productos) {
						...
						for (var j= 0 ; j < id_asignacion.length; j++){
							...
							listas += '<p style="margin-bottom: 3%;">';

							listas += '<span data-nombre-unidad="'+ id_asignacion[j] +'">'+ des_unidad[0] +' </span>';
							...
							listas += '<button type="button" class="btn btn-sm btn-success" data-cotizar="'+ productos[i].id_producto+'" data-id-producto="'+ productos[i].id_producto +'" data-id-asignacion="'+ id_asignacion[j] +'" onclick="cotizar(this);"><span class="glyphicon glyphicon-shopping-cart"></span></button>';
							listas += '</p>';
						}
...
function adicionar_producto(id_producto, id_asignacion) {
	...
	if ($asignacion.size()) {
		...
		plantilla = '<tr class="active" data-producto="' + id_producto + '" data-asignacion="'+id_asignacion+'">' +
						...
						'<td class="align-middle"><input type="text" value="1" name="cantidades[]" class="form-control text-right" maxlength="10" autocomplete="off" data-cantidad="'+id_asignacion+'" data-validation="required number" data-validation-allowing="range[1;1000000]" data-validation-error-msg="Debe ser un número positivo entre 1 y 1000000" onkeyup="calcular_importe(' + id_asignacion + ');"></td>' +

						

						'<td class="align-middle"><input type="text" value="' + unidad + '" class="form-control text-right" autocomplete="off" data-unidad="' + unidad + '"</td>' +
...
function guardar_proforma() {
	...
	$.ajax({
		...
		if (venta) {
			...
			imprimir_proforma(venta);
		} else {
			$('#loader').fadeOut(100);
			$.notify({
				message: 'Ocurrió un problema en el proceso, no se puedo obtener la dosificación ni tampoco guardar los datos de la venta, verifique si la se guardó parcialmente2.'
			}, {
				type: 'danger'
			});
		}
...
function imprimir_proforma(venta) {

	window.open('?/proformas/imprimir/' + venta, '_blank');

	//$.open('?/proformas/imprimir/' + venta, true);
...
function cotizar(elemento) {
	...
	adicionar_producto(cotizar,id_asignacion);
}
...
function sucursales(id) {
	...
	$.ajax({
		url: '?/proformas/cantidad_sucursales/'+id,
		type: 'post',
		data: '',
		async:true,
