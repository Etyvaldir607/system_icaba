<?php
function calcularStock($id_producto,$id_almacen,$id_proforma){
    global $db;
    $query="SELECT IFNULL(I.cantidad_ingresos, 0) AS cantidad_ingresos,
			       IFNULL(E.cantidad_proformas, 0) AS cantidad_proformas
			...
			LEFT JOIN
			  (SELECT d.producto_id,
			          SUM(d.cantidad * u.tamanio) AS cantidad_proformas
			   FROM inv_proformas_detalles d
			   LEFT JOIN inv_proformas e ON (e.id_proforma = d.proforma_id AND id_proforma != '$id_proforma')
			   LEFT JOIN inv_asignaciones a ON a.id_asignacion = d.asignacion_id
			   LEFT JOIN inv_unidades u ON u.id_unidad = a.unidad_id
    ...
    $stock = $resultado != null ? $resultado['cantidad_ingresos']-$resultado['cantidad_proformas']: 0;
    return $stock;
}
...
// $proforma = $db->query('SELECT p.*, a.almacen, a.id_almacen FROM inv_proformas p left JOIN inv_almacenes a ON a.id_almacen = p.almacen_id ')->fetch_first();
...
$pago_simple= $db->select('*, count(id_pago_detalle) as nro_cuotas')
			...
			->where('movimiento_id', $id_proforma)
			->where('tipo', $tipo_)
			->fetch_first();
...
$nro_pagos = $db->select('count(estado) as nro_pagos')
			->from('inv_pagos')
			->join('inv_pagos_detalles d','pago_id = id_pago','left')
			->where('movimiento_id', $id_proforma)
			->where('tipo', $tipo_)
...
$id_almacen = $proforma['almacen_id'];
$almacen 	= $db->from('inv_almacenes')->where('id_almacen', $id_almacen)->fetch_first();
$sucursal 	= $db->from('inv_sucursal')->where('id_sucursal', $id_sucursal)->fetch_first();

$detalles = $db->from('inv_proformas e')
				->join('inv_proformas_detalles d','d.proforma_id = e.id_proforma','left')
				...
				->where(['d.proforma_id'=>$id_proforma,'e.almacen_id'=>$id_almacen])
...
$clientes = $db->query("SELECT id_proforma, nombre_cliente, nit_ci, telefono, direccion, GROUP_CONCAT(b.estado SEPARATOR '|') AS pago FROM inv_proformas c
						LEFT JOIN inv_pagos a ON c.id_proforma = a.movimiento_id
...
<div class="row">
			...
			<div class="panel-body">
				...
				<form id="formulario" class="form-horizontal" data-locked="false">
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" class="form-control" value="<?php echo $sucursal['sucursal']; ?>" disabled="disabled">							
						</div>
						<input type="hidden" value="<?php echo $sucursal['id_sucursal']; ?>" name="sucursal_id" id="sucursal"/>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							...
							<input type="hidden" value="<?= $id_proforma; ?>" name="proforma_id">
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" value="<?= $proforma['telefono'];?>" name="telefono" id="telefono" class="form-control text-uppercase" autocomplete="off" data-validation="letternumber length" data-validation-allowing='-+/.,:;@#&"()_\n ' data-validation-length="max100" data-validation-optional="true" readonly>
						</div>
					</div>
					<div class="form-group">
						<label for="direccion" class="col-sm-4 control-label">Dirección:</label>
						<div class="col-sm-8">
							<input type="text" value="<?= $proforma['direccion'];?>" name="direccion" id="direccion" class="form-control text-uppercase" autocomplete="off" data-validation="letternumber length" data-validation-allowing='-+/.,:;@#&"()_\n ' data-validation-length="max200" data-validation-optional="true" readonly>
						</div>
					</div>
					<div class="form-group">
						<label for="observacion" class="col-sm-4 control-label">Observación:</label>
						<div class="col-sm-8">
							<textarea name="observacion" id="observacion" class="form-control text-uppercase" rows="2" autocomplete="off" data-validation="letternumber" data-validation-allowing='-+/.,:;@#&"()_\n ' data-validation-optional="true" readonly><?= $proforma['observacion']; ?></textarea>
						</div>
					</div>
					...
					<div class="table-responsive margin-none">
						<table id="ventas" class="table table-bordered table-condensed table-striped table-hover margin-none">
							...
							<tbody>
								...
								<?php $totalDescuento 	= $total; //$total - $total * ($proforma['descuento']/100); ?>
								<?php $stock 	= calcularStock($detalle['producto_id'],$id_almacen,$id_proforma); ?>
								<tr class="active" data-producto="<?= $detalle['producto_id']; ?>" data-asignacion="<?= $detalle['asignacion_id']; ?>">
                        			...
                        			<!-- <input type="text" data-tamanio="<?php // $detalle['asignacion_id']; ?>" class="translate" data-validation="required number"></td> -->
                    				...
                    				<td class="text-nowrap align-middle text-center">
                        				<button type="button" class="btn btn-success" tabindex="-1" onclick="eliminar_producto(<?= $detalle['asignacion_id']; ?>)">Eliminar</button>
                    				</td>
                				</tr>
								<?php endforeach ?>
							</tbody>
					...
					<div class="form-group">
						<div class="col-xs-12">
							<input type="text" name="nro_registros" value="" class="translate" tabindex="-1" data-ventas="" data-validation="required number" data-validation-allowing="range[1;<?= $limite_longitud; ?>]" data-validation-error-msg="El número de productos a vender debe ser mayor a cero y menor a <?= $limite_longitud; ?>">
							<input type="text" name="monto_total" value="" class="translate" tabindex="-1" data-total="" data-validation="required number" data-validation-allowing="range[0.01;<?= $limite_monetario; ?>],float" data-validation-error-msg="El monto total de la venta debe ser mayor a cero y menor a <?= $limite_monetario; ?>">
							<input type="text" name="monto_porcentaje" value="" class="translate" tabindex="-1" data-porcentaje="" data-validation="required number" data-validation-allowing="range[0.01;<?= $limite_monetario; ?>],float" data-validation-error-msg="El monto total de la venta debe ser mayor a cero y menor a <?= $limite_monetario; ?>">
							<!--<input type="hidden" value="<?= $proforma['cliente_id']?>" name="cliente_id" id="cliente_id">-->
						</div>
					</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<div class="col-sm-4">
									<div class="radio">
										<label>
											<input type="radio" name="tipo_pago" value="Efectivo" onchange="setPago();" <?php if($detalle['tipo_de_pago']=='Efectivo'){ ?> checked <?php } ?> >
											<span>Efectivo</span>
										</label>
									</div>
								</div>
								<div class="col-sm-4">
									<div class="radio">
										<label>
											<input type="radio" name="tipo_pago" value="Tarjeta" onchange="setPago();" <?php if($detalle['tipo_de_pago']=='Tarjeta'){ ?> checked <?php } ?> >
											<span>Tarjeta</span>
										</label>
									</div>								
								</div>
								<div class="col-sm-4">
									<div class="radio">
										<label>
											<input type="radio" name="tipo_pago" value="Deposito" onchange="setPago();" <?php if($detalle['tipo_de_pago']=='Deposito'){ ?> checked <?php } ?> >
											<span>Deposito</span>
										</label>
									</div>								
								</div>
								...
								<input type="text" 
								<?php if($detalle['tipo_de_pago']=='Efectivo' || $detalle['tipo_de_pago']=='Tarjeta' || $detalle['tipo_de_pago']=='Deposito'){ ?> 
									value="2" 
								<?php }else{ ?> 
									value="0" 
								<?php } ?> 
								id="data-tipo-pago" class="translate" data-validation="required number" data-validation-allowing="range[1;2],float" data-validation-error-msg="No eligio el modo de pago">
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						<div class="col-xs-12">
							...
							<input type="text" id="monto_plan_pagos" name="monto_plan_pagos" value="1" class="translate" tabindex="-1" data-total-pagos="" data-validation="required number" data-validation-allowing="range[0.01;1000000.00],float" data-validation-error-msg="La suma de las cuotas debe ser igual al costo total de la venta">
						</div>
					</div>
	...
	<div class="col-md-6">
		...
		<div class="row">
			<div class="col-xs-12 text-right">
				<a href="?/operaciones/proformas_listar" class="btn btn-primary"><span class="glyphicon glyphicon-list"></span><span class="hidden-xs"> Listar</span></a>
			</div>
		</div>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<p class="text-right">
					<a href="?/operaciones/proformas_listar" class="btn btn-warning">Listado</a>
				</p>
				<?php endif ?>
				<form method="post" action="?/operaciones/buscar/<?php echo $id_almacen.'/'.$id_proforma; ?>" id="form_buscar_0" class="margin-bottom" autocomplete="off">
				...
				<form method="post" action="?/operaciones/buscar/<?php echo $id_almacen.'/'.$id_proforma; ?>" id="form_buscar_1" class="margin-bottom" autocomplete="off">
					<div class="form-group has-feedback">
...
function eliminar_producto(id_asignacion) {
	bootbox.confirm('Está seguro que desea eliminar el producto?', function (result) {
	    if(result){
	       ...
	       // alert('pasó por eliminar_producto');
	        renumerar_productos();
	        calcular_total();
	    }
	});
...
function calcular_total() {
	...
	$importes.each(function (i) {
		    ...
		    total = total + importe;
		    //alert('pasó por calcular total ['+total+']');
...
function guardar_nota(form) {
	...
	if ($form.data('locked') != undefined && !$form.data('locked')) {
		...
		$.ajax({
		    type: 'post',
		    dataType: 'json',
		    url: '?/operaciones/guardar_remision',
		    data: data,
		    beforeSend: function() {
		    ...
		    if (venta) {
		        ...
		        window.location.href="?/operaciones/proformas_listar";
...
function actualizar(elemento) {
	...
	$.ajax({
	    type: 'post',
	    dataType: 'json',
	    url: '?/operaciones/actualizar', //notas/actualizar',
	    data: {
...
function set_cuotas_val() {
	...
	var limite_segun_BD=<?php echo $nro_pagos['nro_pagos'] ?>;
	var cantidad = $('#nro_cuentas').val();
	var total = 0;

	for(i=1;i<=limite_segun_BD && i<=cantidad;i++){
	    importe=$('[data-cuota=' + i + ']').children('.monto_cuota').val();
	    importe = parseFloat(importe);
	    total = total + importe;
	}

	nro 	= $('#nro_cuentas').val()-limite_segun_BD;
	valor 	= parseFloat($('[data-subporcentaje]:first').text())-total;
	valor 	= valor/nro;

	nro 	= $('#nro_cuentas').val();
	//alert(valor+" "+nro);
	
	for(i = limite_segun_BD+1;i<=nro;i++){
