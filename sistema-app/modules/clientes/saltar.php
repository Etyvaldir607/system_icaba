...
if ($producto) {
	...
	/*	
	if ($tipo == 'antes') {
		$id_producto = $db->query("	SELECT ifnull(max(id_producto), (select max(id_producto) from inv_productos)) as id_producto 
									from inv_productos 
									where id_producto < '$id_producto'
									")->fetch_first();
		...
		$id_producto = $db->query("	SELECT ifnull(min(id_producto), (select min(id_producto) from inv_productos)) as id_producto 
									from inv_productos 
									where id_producto > '$id_producto'
									")->fetch_first();
