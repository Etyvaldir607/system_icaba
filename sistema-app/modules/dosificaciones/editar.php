		...
		<div class="col-sm-9 col-sm-offset-1">
			<form method="post" action="?/dosificaciones/guardar" class="form-horizontal">
				<div class="form-group">
					<label for="nro_tramite" class="col-md-4 control-label">Punto de venta:</label>
					<div class="col-md-8">
						<select name="cmb_sucursal" id="cmb_sucursal" class="form-control">
							<option value=""></option>
							<?php
							$almacenes = $db->select('*')
											->from('inv_sucursal')
							...
							foreach ($almacenes as $nro => $almacen) { ?>
								<option value="<?php echo $almacen['id_sucursal']; ?>"><?php echo $almacen['sucursal']; ?></option>
							<?php } ?>
						</select>	
					</div>
				</div>				
...
<script>
$(function () {
	$("#cmb_sucursal option[value='<?php echo $dosificacion['sucursal_id']; ?>']").attr("selected",true);		
