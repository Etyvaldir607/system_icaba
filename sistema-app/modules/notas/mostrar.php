...
$ventas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')
                ...
                ->where('i.empleado_id', $_user['persona_id'])->where('i.tipo', 'Venta')
...
$permiso_editar 	= in_array('modificar', $permisos);
				...
				<td class="text-nowrap">
					...
					<?php if ($permiso_ver) : ?>
					    <?php if($venta['plan_de_pagos'] == 'si'){ ?>
						    <a href="?/cobrar/notas_ver/<?= $venta['id_egreso']; ?>" class="btn btn-danger" class="btn btn-default">Cobrar deuda</a>
					    <?php } ?>
					<?php endif ?>
				</td>
				<?php } ?>
