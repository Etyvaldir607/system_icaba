...
if (!$_is_module && module != home && module != tools) {
	...
	if (!in_array(file, $_views) && module != home && module != tools) {
		// Error 401
		require_once bad_request();
		exit;
	}
...
$_user = $db->select('u.*, r.rol, p.paterno, p.materno, p.nombres, r.incremento')->from('sys_users u')->join('sys_roles r', 'u.rol_id = r.id_rol', 'left')->join('sys_empleados p', 'u.persona_id = p.id_empleado', 'left')->where('u.id_user', $_SESSION[user]['id_user'])->fetch_first();
