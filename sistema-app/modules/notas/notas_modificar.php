    ...
    $query="SELECT IFNULL(I.cantidad_ingresos, 0) AS cantidad_ingresos,
			...
			LEFT JOIN
			  (SELECT d.producto_id,
			   ...
			   LEFT JOIN inv_egresos e ON (e.id_egreso = d.egreso_id AND estado = 'V' AND id_egreso != '$id_egreso')
			   LEFT JOIN inv_asignaciones a ON a.id_asignacion = d.asignacion_id
			   LEFT JOIN inv_unidades u ON u.id_unidad = a.unidad_id
...
$egreso = $db->query("SELECT e.* , ca.id_categoria_cliente, ca.categoria
                    from inv_egresos as e
                    LEFT join inv_sucursal  as s ON e.sucursal_id = s.id_sucursal
                    LEFT JOIN inv_clientes as c ON c.id_cliente = e.cliente_id
                    LEFT JOIN inv_categorias_cliente as ca ON ca.id_categoria_cliente = c.categoria_cliente_id
...
<div class="row">
			...
			<div class="panel-body">
				...
				<form id="formulario" class="form-horizontal" data-locked="false">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" value="<?= $egreso['categoria'];?>" name="categoria" id="categoria" class="form-control text-uppercase" autocomplete="off" readonly>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="table-responsive margin-none">
						<table id="ventas" class="table table-bordered table-condensed table-striped table-hover margin-none">
							...
							<tbody>
								...
								<?php $totalDescuento 	= $total - $total * ($egreso['descuento']/100); ?>
								...
								<tr class="active" data-producto="<?= $detalle['producto_id']; ?>" data-asignacion="<?= $detalle['asignacion_id']; ?>">
                    				<td class="text-nowrap align-middle">
                    					...
                    					<b><?= ($key+1); ?></b>
                    					<input type="hidden" value="0" class="detallesEliminar" name="detallesEliminar[]"> <!-- ya no es necesario, eliminar -->
                    				</td>
	...
	<div class="col-md-6">
		...
		<div class="row">
			<div class="col-xs-12 text-right">
				<a href="?/operaciones/notas_listar" class="btn btn-primary"><span class="glyphicon glyphicon-list"></span><span class="hidden-xs"> Listar</span></a>
			</div>
		</div>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<p class="text-right">
					<a href="?/operaciones/notas_listar" class="btn btn-warning">Listado</a>
				</p>
				<?php endif ?>
				<form method="post" action="?/notas/buscar/<?php echo $id_almacen.'/'.$id_egreso; ?>" id="form_buscar_0" class="margin-bottom" autocomplete="off">
				...
				<form method="post" action="?/notas/buscar/<?php echo $id_almacen.'/'.$id_egreso; ?>" id="form_buscar_1" class="margin-bottom" autocomplete="off">
					<div class="form-group has-feedback">
