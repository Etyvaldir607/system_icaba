...
$movimientos = $db->query("select m.*, ifnull(concat(e.nombres, ' ', e.paterno, ' ', e.materno), '') as empleado 
							from (select i.id_ingreso as id_movimiento, d.id_detalle, i.fecha_ingreso as fecha_movimiento, i.hora_ingreso as hora_movimiento, i.descripcion, d.cantidad, d.costo as monto, d.descripcion as descripcion_asignacion, 'i' as tipo, i.empleado_id, i.almacen_id 
									from inv_ingresos_detalles d left join inv_ingresos i on d.ingreso_id = i.id_ingreso 
									where d.producto_id = $id_producto union 
																	select e.id_egreso as id_movimiento, d.id_detalle, e.fecha_egreso as fecha_movimiento, e.hora_egreso as hora_movimiento, e.descripcion, d.cantidad, d.precio as monto, d.descripcion as descripcion_asignacion, 'e' as tipo, e.empleado_id, e.almacen_id 
		...
		<table class="table table-bordered table-condensed table-striped table-hover">
			<thead>
				<tr class="active">
					...
					<th class="text-nowrap text-center align-middle" colspan="3">Entradas</th>
					<th class="text-nowrap text-center align-middle" colspan="3">Salidas</th>
					<th class="text-nowrap text-center align-middle" colspan="3">Saldos</th>
			...
			<tbody>
				...
				<?php foreach ($movimientos as $nro => $movimiento) { ?>
					...
					<?php $saldo_cantidad = $saldo_cantidad + $movimiento['cantidad']; ?>
					<?php $saldo_costo = $saldo_costo + ($movimiento['cantidad'] * $movimiento['monto']); ?>
					<tr>
