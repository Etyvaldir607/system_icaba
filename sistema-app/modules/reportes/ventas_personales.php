...
// Obtiene las ventas
$ventas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')->from('inv_egresos i')->join('inv_almacenes a', 'i.almacen_id = a.id_almacen', 'left')->join('sys_empleados e', 'i.empleado_id = e.id_empleado', 'left')->where('i.empleado_id', $_user['persona_id'])->where('i.tipo', 'Venta')->where('i.fecha_egreso >= ', $fecha_inicial)->where('i.fecha_egreso <= ', $fecha_final)->order_by('i.fecha_egreso desc, i.hora_egreso desc')->fetch();
...
<div class="panel-body">
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-nowrap"><?php 
					echo escape($venta['tipo']).' ';
					if($venta['codigo_control'] != ''){
						echo "computarizada";
					}
					else{
						if($venta['provisionado'] == 'S'){
							echo "con nota de remisión";
						}
						else{
							echo "manual";
						}
	...
	<div class="alert alert-danger">
		...
		<p>No existen ventas computarizadas registrados en la base de datos.</p>
	</div>
	<?php } ?>
</div>
...
$(function () {
	...
	var table = $('#table').DataFilter({
		filter: true,
		name: 'reporte_ventas_personales',
