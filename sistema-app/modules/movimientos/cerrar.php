...
// Obtiene las ventas
$ventas = $db->query("SELECT * FROM inv_egresos e WHERE e.fecha_egreso = '$fecha' AND e.tipo='Venta' AND e.estado = 'V' AND e.empleado_id = '" . $_user['persona_id'] . "' group by id_egreso")->fetch();
//Obtiene compras
...
$cronogramas = $db->query("select c.periodo, cc.detalle, cc.monto, cc.fecha_pago, cc.id_cronograma_cuentas, cc.tipo_pago 
						from cronograma c left join cronograma_cuentas cc on c.id_cronograma = cc.cronograma_id 
...
//pagos compras
$pagos_compras = $db->query("select p.id_pago, p.movimiento_id, i.fecha_ingreso, i.fecha_ingreso, i.tipo, i.id_ingreso, i.nombre_proveedor, i.monto_total, pd.tipo_pago, ifnull(monto,0) as subtotal 
							from inv_pagos p 
							left join inv_pagos_detalles pd on p.id_pago= pd.pago_id 
										...
										<?php foreach ($ventas as $key => $venta) { ?>
											<tr>
												...
												<?php if ($venta['plan_de_pagos'] == 'si'){ ?>
													...
													<?php $pagos = $db->query("select id_pago, movimiento_id, tipo_pago, ifnull(pd.monto,0) as subtotal 
																		from inv_pagos p 
                                                                        left join inv_pagos_detalles pd on p.id_pago= pd.pago_id 
																		where p.movimiento_id = '". $venta['id_egreso'] . "' AND pd.estado='1' AND p.tipo='Egreso' 
																		GROUP by movimiento_id")->fetch_first(); ?>
																		
																		<!--select id_pago, movimiento_id, tipo_pago, ifnull(SUM(monto),0) as subtotal -->
												...
												<?php }else{ ?>
													<?php if ($venta['tipo_de_pago'] == 'Efectivo'){ ?>
														...
														$monto_total = $monto_total*(1-$descuento/100); 
														$total_venta = $total_venta + $monto_total; 
														 
														?>
														<td class="text-nowrap text-middle text-right"><?= number_format($monto_total, 2, '.', ''); ?></td>
														<td class="text-nowrap text-middle text-right"><?= number_format($monto_total, 2, '.', ''); ?></td>
											...
											<?php foreach ($compras as $key => $compra) { ?>
												<tr>
													...
													<?php if ($compra['plan_de_pagos'] == 'si'): ?>
														...
														<?php $pagos = $db->query("select id_pago, movimiento_id, pd.tipo_pago, ifnull(SUM(monto),0) as subtotal from inv_pagos p left join inv_pagos_detalles pd on p.id_pago= pd.pago_id where p.movimiento_id = '". $compra['id_ingreso'] . "' AND p.tipo='Ingreso' AND pd.estado='1' GROUP by movimiento_id")->fetch_first(); ?>
														<?php if ($pagos){ ?>
															<?php 
															if ($pagos['tipo_pago'] == 'Efectivo') 
