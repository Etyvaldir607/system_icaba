...
$productos = $db->query("SELECT
                                ...
                                WHERE p.codigo like '%" . $busqueda . "%' OR p.nombre like '%" . $busqueda . "%' OR p.codigo_barras like '%" . $busqueda . "%' OR c.categoria like '%" . $busqueda . "%' order by c.orden ASC, p.codigo")->fetch();
				...
				<form method="post" action="?/egresos/guardar" id="formulario" class="form-horizontal">
					...
					<div class="form-group">
						<div class="col-xs-12">
							...
							<input type="text" name="nro_registros" value="0" class="translate" tabindex="-1" data-ventas="" data-validation="required number" data-validation-allowing="range[1;500]" data-validation-error-msg="El número de productos a vender debe ser mayor a cero y menor a 20">
			...
			<div class="panel-body">
				...
				<table id="productos" class="table table-bordered table-condensed table-striped table-hover table-xs">
					...
					<tbody>
						...
						<?php $asignaciones = $db->select('*')->from('inv_asignaciones a')->join('inv_unidades u','a.unidad_id=u.id_unidad')->where('a.producto_id',$producto['id_producto'])->fetch();?>
						<tr>
