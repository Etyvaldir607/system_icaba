...
// Obtiene las ventas
$ventas = $db->query("SELECT v.*, p.nombre, p.nombre_factura, concat(e.nombres, ' ', e.paterno, ' ', e.materno) as empleado, c.categoria
					from (
						select 	e.fecha_egreso, e.hora_egreso, e.nro_factura, e.nombre_cliente, e.nit_ci, 
								e.empleado_id, d.*, e.descuento as descuento_total, u.unidad 
						from inv_egresos_detalles d 
						left join inv_egresos e on (d.egreso_id = e.id_egreso) 
						left join inv_asignaciones ag on ag.id_asignacion=d.asignacion_id
						left join inv_unidades u on u.id_unidad=ag.unidad_id
						where e.tipo = 'venta' AND e.estado = 'V'
					) v 
					...
					where v.fecha_egreso between '$fecha_inicial' and '$fecha_final'
					")->fetch();
	...
	<table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
		...
		<tfoot>
			<tr class="active">
				...
				<th class="text-nowrap align-middle" data-datafilter-filter="true">Factura</th>
				<th class="text-nowrap align-middle" data-datafilter-filter="true">Producto</th>
				...
				<th class="text-nowrap align-middle" data-datafilter-filter="true">Importe <?= escape($moneda); ?></th>
		...
		<tbody>
			...
			<?php $cantidad = escape($venta['cantidad']); ?>
			<?php $precio = escape($venta['precio']); ?>
			<?php $unidad = escape($venta['unidad']); ?>
			<?php $importe = ($cantidad * $precio)*(1-$venta['descuento_total']/100); ?>
			<?php $total = $total + $importe; ?>
			<tr>
				<th class="text-nowrap"><?= $nro + 1; ?></th>
				<td class="text-nowrap">
					<span><?= escape(date_decode($venta['fecha_egreso'], $_institution['formato'])); ?></span>
				...
				<td class="text-nowrap">
					<b><?= escape($venta['categoria']); ?></b>
				</td>
				<td class="text-nowrap text-right"><?= $cantidad; ?></td>
				<td class="text-nowrap text-right"><?= $unidad; ?></td>
				<td class="text-nowrap text-right"><?= $precio; ?></td>
				<td class="text-nowrap text-right"><?= $venta['descuento_total']; ?></td>
