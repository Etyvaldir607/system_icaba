...
// Obtiene las ventas
$ventas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')->from('inv_egresos i')->join('inv_almacenes a', 'i.almacen_id = a.id_almacen', 'left')->join('sys_empleados e', 'i.empleado_id = e.id_empleado', 'left')->where('i.empleado_id', $_user['persona_id'])->where('i.tipo', 'Venta')->where('i.codigo_control != ', '')->where('i.provisionado', 'N')->where('i.fecha_egreso >= ', $fecha_inicial)->where('i.fecha_egreso <= ', $fecha_final)->order_by('i.fecha_egreso desc, i.hora_egreso desc')->fetch();
...
<div class="panel-heading" data-formato="<?= strtoupper($formato_textual); ?>" data-mascara="<?= $formato_numeral; ?>" data-gestion="<?= date_decode($gestion_base, $_institution['formato']); ?>">
	<h3 class="panel-title">
		...
		<strong>Ventas computarizadas personales</strong>
	</h3>
</div>
<div class="panel-body">
	...
	<div class="row">
		<div class="col-sm-8 hidden-xs">
			<div class="text-label">Para realizar una venta computarizada, hacer clic en el siguiente botón: </div>
		</div>
		<div class="col-xs-12 col-sm-4 text-right">
			...
			<a href="?/electronicas/crear" class="btn btn-primary"><span class="glyphicon glyphicon-plus"></span><span class="hidden-xs"> Venta computarizada</span></a>
			<?php } ?>
		</div>
	</div>
	<hr>
	...
	<table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-nowrap">Ventas Computarizadas</td>
				...
				<td class="text-nowrap">
					<?php if ($permiso_ver) { ?>
					<a href="?/electronicas/ver/<?= $venta['id_egreso']; ?>" data-toggle="tooltip" data-title="Ver detalle de la venta computarizada"><i class="glyphicon glyphicon-list-alt"></i></a>
					<?php } ?>
					<?php if ($permiso_eliminar) { ?>
					<a href="?/electronicas/eliminar/<?= $venta['id_egreso']; ?>" data-toggle="tooltip" data-title="Eliminar venta computarizada" data-eliminar="true"><span class="glyphicon glyphicon-trash"></span></a>
					<?php } ?>
				</td>
				<?php } ?>
			</tr>
	...
	<div class="alert alert-danger">
		...
		<p>No existen ventas computarizadas registradas en la base de datos.</p>
	</div>
	<?php } ?>
</div>
...
$(function () {	
	...
	$('[data-eliminar]').on('click', function (e) {
		...
		bootbox.confirm('Está seguro que desea eliminar la venta computarizada y todo su detalle?', function (result) {
			if(result){
				window.location = url;
			}
		});
