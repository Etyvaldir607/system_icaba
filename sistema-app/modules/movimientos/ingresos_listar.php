...
$ingresos = $db->select("m.*, upper(IFNULL(a.sucursal, 'sucursal no registrado')) AS sucursal, concat(e.nombres, ' ', e.paterno, ' ', e.materno) as empleado")
				->from('caj_movimientos m')
				->join('sys_empleados e', 'm.empleado_id = e.id_empleado', 'left')
				->join('inv_sucursal a', 'm.sucursal_id = a.id_sucursal', 'left')
				->where('m.tipo', 'i')
	...
	<table id="table" class="table table-bordered table-condensed table-striped table-hover">
		...
		<tbody>
			...
			<tr>
				...
				<?php if ($permiso_modificar || $permiso_eliminar || $permiso_imprimir) : ?>
				<td class="text-nowrap">
					<?php $masUnMesdate = date("Y-m-d",strtotime($ingreso['fecha']."+ 1 month")); ?>
					<?php if ($permiso_modificar && date('Y-m-d') <= $masUnMesdate) : ?>
					<a href="?/movimientos/ingresos_modificar/<?= $ingreso['id_movimiento']; ?>" data-toggle="tooltip" title="Puede editar el movimiento hasta la fecha: <?= $masUnMesdate ?>"><i class="glyphicon glyphicon-edit"></i></a>
					<?php endif ?>
					<?php if ($permiso_eliminar && date('Y-m-d') <= $masUnMesdate) : ?>	
					<a href="?/movimientos/ingresos_eliminar/<?= $ingreso['id_movimiento']; ?>" data-toggle="tooltip" title="Puede eliminar el movimiento hasta la fecha: <?= $masUnMesdate ?>" data-eliminar="true"><i class="glyphicon glyphicon-trash"></i></a>
					<?php endif ?>
